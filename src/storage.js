...
/** 할 일, 메모, 세부분류, 자동 백업 설정을 사람이 읽기 좋은 JSON 문자열로 직렬화한다. */
export function serializeExport(tasks, memo = "", subcategories = {}, backupConfig = null) {
 ...
 * @returns {{ ok: true, tasks: Array, memo: string, subcategories: object, backupConfig: object|null } | { ok: false, error: string }}
 */
export function parseImport(text) {
  ...
  const subcategories = data?.subcategories && typeof data.subcategories === "object"
    ...
    : null;
  const backupConfig = data?.backupConfig && typeof data.backupConfig === "object"
    ? data.backupConfig
    : null;
  return { ok: true, tasks, memo, subcategories, backupConfig };
}
